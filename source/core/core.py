...
class Core(DataCore):
    ...
    def add_payment_to_json(self, method: PaymentMethod, montant: Decimal, itemsVendu: list[ProduitData]):
        print(__file__)
        chemin_absolu: str = __file__[:-len(os.path.basename(__file__))]
        with open(chemin_absolu + "Payment.json", "r", encoding="utf8") as file:
            data = js.load(file)
            file.close()
        ...
        if today in data:
            ...
            data[today]["payment"] = {}
            data[today]["payment"]["MethodPayment"] = method
