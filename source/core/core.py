...
def enregistrer_commande(methode: MethodesPayment, 
    ...
    """
    chemin_absolu: str = sys.path[0]
    today: str = dt.today().strftime("%d/%m/%Y")

    with open(chemin_absolu + "\\commands.json", "r", encoding="utf8") as file:
        data = js.load(file)
        file.close()
    if today in data:
        data[today] = {}
        data[today]["amount"] = montant 
        data[today]["payment"] = {}
        data[today]["payment"]["MethodPayment"] = str(methode)
        data[today]["payment"]["amount"] = montant
        data[today]["payment"]["hour"] = dt.now().strftime("%H")
        data[today]["payment"]["item"] = [p.nom for p in produits]
        print(data)
    else:
        data[today] = {}
        data[today]["amount"] = montant 
...
#     def add_payment_to_json(self, method: MethodesPayment, montant: Decimal, itemsVendu: list[ProduitData]):
#         chemin_absolu: str = sys.path[0]
#         with open(chemin_absolu + "\\commands.json", "r", encoding="utf8") as file:
#             data = js.load(file)
#             file.close()
...
#             data[today]["amount"] = montant 
#             data[today]["payment"] = {}
#             data[today]["payment"]["MethodPayment"] = str(method)
#             data[today]["payment"]["amount"] = montant
#             data[today]["payment"]["hour"] = dt.now().strftime("%H")
#             data[today]["payment"]["item"] = [p.nom for p in itemsVendu]
#             print(data)
#         else:
#             data[today] = {}
#             data[today]["amount"] = montant 
